...
class Login extends CI_Controller {
	...
	public function quiz_user_details(){
	    header('Content-type: application/json');
	    try{
         $this->form_validation->set_rules('quiz_name', 'Name', 'trim|xss_clean|required');
         $this->form_validation->set_rules('quiz_email', 'Email ID', 'trim|xss_clean|required|valid_email');
         $this->form_validation->set_rules('quiz_phone', 'Mobile', 'trim|xss_clean|required|min_length[10]');
         ...
         exit();
             ...
             'email'=> $this->input->post('quiz_email'),
             'mobile'=> $this->input->post('quiz_phone')
          );
                $this->session->set_userdata('quiz_user_details', $customer_details);
                echo json_encode(array("code"=>200,"status1"=>"sucess","message"=>"Detalis submitted successfully!"));
                return true;
                exit();
	   }catch(Exception $ex){
	       header('Content-type: application/json');
	...
	public function forgot_password(){
	    ...
	    if(isset($this->session->userdata['logged_in'])){
		redirect(base_url()."my-account",$data);
        }else{
                $this->form_validation->set_rules('user_name', 'User Name/Email', 'trim|required|xss_clean');
                $this->data['message_error'] = ($this->form_validation->run() == False) ? validation_errors() : $this->session->flashdata('message');
        
                 if ($this->form_validation->run() == FALSE) {
                 $this->load->view('login/forgot_password_page',$data);
                 return false;
                 }
                 
                if($this->input->post('user_name')){
                    $request_details = $this->login_form->Check_Email_Forgot($this->input->post('user_name'));
                            if ($request_details && is_array($request_details)){
                                    
                                    $randomletter = substr(str_shuffle("1234567890abcdefghijklmnopqrstuvwxyz"), 0, 8);
                                    $request_details = $this->login_form->Reset_Password(array('email'=>$this->input->post('user_name'),'password'=>$randomletter));
                                    
                                    $subject='Password Resetted';
                                    $message='New password is : '.$randomletter.'<br> Use this password to login into iasshiksha.com website.';
                                    $this->Mailsend->send_mail('info@iasshiksha.com',$this->input->post('user_name'),$subject,$message);
                                    
                                   $this->session->set_flashdata('message', 'The New password has been sent to your registered Email ID!');
                                   $this->data['message'] = $this->session->flashdata('message');
                                   redirect(base_url().'forgot_password');
                            }else{
                                $this->session->set_flashdata('message', 'This Email ID is not registered');
                                $this->data['message'] = $this->session->flashdata('message');
                            }
                }
