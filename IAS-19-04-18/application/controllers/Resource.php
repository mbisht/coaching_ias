...
class Resource extends CI_Controller {
	...
	public function start_online_test($arg){
	    ...
	    $data['alert_notification_data']=$this->All_page->GetAlertNotification();
	    $data['activepage']="online-test";
	    $data['question_type']=$arg;
	    
	    if(isset($this->session->userdata['logged_in'])){
	    $data['GetUserDetails'] = $this->All_page->GetUserDetail($this->session->userdata['logged_in']['id']);
	    }else{
	    $data['GetUserDetails'] = array();
	    }
	    
        	if(isset($this->session->userdata['logged_in']) && in_array("1", explode(",",$data['GetUserDetails'][0]['role']))){
        	    $data['online_test_prelims_submitted']=$this->All_page->GetOnlineTestPrelimsSubmitted(array("user_id"=>$this->session->userdata['logged_in']['id'],"quetion_id"=>$arg));
        	    if(!$data['online_test_prelims_submitted']){
        	        ...
        	        $data['GetSpecialUserForPrelims']=$this->All_page->GetSpecialUserForPrelims(array("user_id"=>$this->session->userdata['logged_in']['id'],"quetion_id"=>$arg));
        	        if($data['GetSpecialUserForPrelims'] && $data['GetSpecialUserForPrelims']['0']['status'] >= count($data['online_test_prelims_submitted'])){
            		 $data['online_test_prelims_data']=$this->All_page->GetOnlineTestPrelimsQues($arg);
            		 $this->load->view('resource/online_test_prelims_start',$data);
        	        }else{
        	        $data['alert']="Already you had attended this Test. One user can attend test only one time!";
                    $this->load->view('common/permission_denied',$data);
        	        }
                }
        }else{
    ...
    public function print_prelims(){
	    ...
	    if(!empty($user_id) && !empty($question_set)){
	        $data['getUserDetails']=$this->All_page->getUserDetails($user_id);
	        $data['lastPrelimsResult']=$this->All_page->lastPrelimsResult($user_id,$question_set);
	        if(!empty($data['lastPrelimsResult']) && is_array($data['lastPrelimsResult']) && $data['lastPrelimsResult'][0]['question_id']){
	        $data['lastPrelimsQuestions']=$this->All_page->lastPrelimsQuestions($data['lastPrelimsResult'][0]['question_id']);
	        $this->load->view('resource/print_prelims',$data);
	        }else{
	        redirect(base_url().'online-test-prelims', 'refresh');
	        }
	...
	public function daily_currentaffair(){
	    $data['notification_data']=$this->All_page->GetNotificationDetails();
	    $data['alert_notification_data']=$this->All_page->GetAlertNotification();
		$data['activepage']="current-affair";
		$data['WeeklyCurrentAffair']=$this->All_page->GetDailyCurrentAffair();
		$this->load->view('resource/daily_currentaffair',$data);
	}
	
	public function load_daily_currentaffair(){
	    $data['notification_data']=$this->All_page->GetNotificationDetails();
	    $data['alert_notification_data']=$this->All_page->GetAlertNotification();
	    header('Content-type: application/json');
        ini_set('memory_limit', '-1');
        ...
        $customer_details = array('date' => $_REQUEST['date']);
	    $quiz_details = $this->All_page->GetDailyCurrentAffairByDate($customer_details['date']);
	        if ($quiz_details && is_array($quiz_details)){
            echo json_encode(array("code"=>"200","status1"=>"succes","message"=>"Data Available!","items"=>$quiz_details));
            return true;
            }else{
