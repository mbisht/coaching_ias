...
class All_page extends CI_Model{
    ...
    public function lastPrelimsResult($user_id,$question_type){
    $sql = "SELECT * FROM online_test_submit_prelims WHERE user_id='$user_id' AND question_type='$question_type' ORDER BY date_added DESC LIMIT 1";
    $query = $this->db->query($sql);        
    return $query->result_array();
    }
    
    public function lastPrelimsQuestions($data){
    $question_id = json_decode($data);
    $result = array();
    foreach($question_id as $key=>$value){
    $sql = "SELECT * FROM online_test_questions_prelims WHERE id='$value'";
    $query = $this->db->query($sql);        
    $result[$value] = $query->result_array();
    }
    return $result;
    }
    ...
    public function GetTodayQuiz(){
        $sql = "SELECT * FROM quiz_questions WHERE date=(SELECT date FROM quiz_questions ORDER BY date_added DESC LIMIT 1) ORDER BY date_added ASC LIMIT 10";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
    
    public function GetQuizDates(){
        $sql = "SELECT DISTINCT date FROM quiz_questions ORDER BY date_added ASC";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
    
    public function GetDateQuiz($data){
        $sql = "SELECT * FROM quiz_questions WHERE date='$data' ORDER BY date_added ASC";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
    ...
    public function GetDailyCurrentAffair(){
        $sql = "SELECT * FROM daily_current_affairs ORDER BY date_added DESC";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
    
    public function GetDailyCurrentAffairByDate($data){
        $sql = "SELECT * FROM daily_current_affairs WHERE date='$data' ORDER BY date_added DESC";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
    ...
    public function GetOnlineTestPrelims(){
        ...
        $sql_result = "SELECT question_type FROM online_test_questions_prelims otp INNER JOIN online_test_prelims_question_set ots ON ots.question_set=otp.question_type WHERE otp.subject_id='$value[id]' AND ots.status='1' GROUP BY otp.question_type ORDER BY otp.question_type ASC";
        $query_result = $this->db->query($sql_result);   
    ...
    public function GetOnlineTestPrelimsQues($data){
        $sql = "SELECT * FROM online_test_questions_prelims WHERE question_type='$data' ORDER BY date_added ASC LIMIT 100";        
        $query = $this->db->query($sql);        
        return $query->result_array();
    }
}