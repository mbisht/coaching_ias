		...
		<h3>Reset Password</h3>
            ...
            <div class="alert alert-success" style="<?php if($this->session->flashdata('message')){ echo 'display: block;'; }else{ echo 'display: none;'; } ?>"><?php if($this->session->flashdata('message')){ echo $this->session->flashdata('message'); } ?></div>
                    <form method="POST" enctype="multipart/form-data" action="/forgot_password" >
                        ...
                        <input type="text" name="user_name" id="login_user_name" value="<?php if(set_value('user_name')) {echo set_value('user_name'); } ?>" class="input form-control" placeholder="Email ID" required >
                        </div>
                        <div class="form-group">
                        <input type="submit" name="login_to_client" class="submit btn btn-flat btn-theme-colored text-uppercase mt-20 mb-sm-30 border-left-theme-color-2-4px" value="Send Request" id="login_to_client_login">
                        </div>
                        </div>
                    </form>
            </div>
