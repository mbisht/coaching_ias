		...
		<div class="container">
    		...
    		<div class="account-details table-style">
    		   <table class="table jsgrid table-striped table-bordered dataTable table-responsive">
								<thead>
								    <tr><th colspan="7">Mains Test Result</th></tr>
									<tr>
										<th>Sno.</th>
										<th>Question</th>
										<th>Subjects</th>
								...
								<?php
								$count=1;
								if(is_array($getMainsDetails)){
                                foreach($getMainsDetails as $value){
                                ?><tr>
								<td><?php echo $count;?></td>
								<td><?php echo $value['title'];?></td>
								<td><?php echo $value['subject'];?></td>
									<td><a href="<?php echo base_url(); ?>assets/files/mains/<?php echo $value['ResultFile'];?>" target="_blank"><i class="fa fa-download" aria-hidden="true"></i> Download</a></td>
									<td><?php if(!empty($value['corrected_file'])){ ?><a href="<?php echo base_url(); ?>assets/files/mains/<?php echo $value['corrected_file'];?>" target="_blank"><i class="fa fa-download" aria-hidden="true"></i> Download</a> <?php } ?></td>
