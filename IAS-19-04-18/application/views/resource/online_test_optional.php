...
<section class="content online-test">
		<div class="container">
            <div>
                <h1 class="mt-0 text-uppercase font-28 line-bottom line-height-1">ONLINE TEST <span class="text-theme-color-2 font-weight-400">Optional</span></h1>
                    <div class="col-xs-12 news-container">
                        ...
                        <div id="home" class="tab-pane fade in active">
                    		        <?php
                    		        if(isset($online_test_optional_data['subjects']) && is_array($online_test_optional_data['subjects'])){
                    		        foreach($online_test_optional_data['subjects'] as $key_sub=>$value_sub){ ?>
                    		        <div class="col-md-4 col-sm-6 col-xs-12">
                    		            <h3><?php echo $value_sub; ?></h3>
                    		            <ul class="list file-list">
                    		                <?php
                    		                foreach($online_test_optional_data[$value_sub] as $key=>$value){ ?>
                    		                <li><?php echo "<a href='".base_url()."assets/files/optional/".$value['file_url']."' title='$value[title]' target='_blank'>".$value['title']."</a>"; ?></li>
                    		                <?php } ?>
                    		            </ul>
                    		        </div>
    ...
    <div class="alert alert-success" style="<?php if($this->session->flashdata('message')){ echo 'display: block;'; }else{ echo 'display: none;'; } ?>"><?php if($this->session->flashdata('message')){ echo $this->session->flashdata('message'); } ?></div>
      ...
      <form method="post" action="<?php echo base_url(); ?>online-test-optional-submit" name="onlineTestSubmit" id="onlineTestSubmit" enctype="multipart/form-data">
